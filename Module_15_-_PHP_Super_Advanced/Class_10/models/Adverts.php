...
class Adverts extends model {
	...
	public function getTotalAdverts($filters) {
		...
		$sql = $this->db->prepare("SELECT COUNT(*) as c FROM adverts WHERE ".implode(' AND ', $filterstring));
	...
	public function getMyAdverts() {
		...
		$array = array();
		$sql = $this->db->prepare("SELECT *, (SELECT adverts_images.url FROM adverts_images WHERE adverts_images.id_advert = adverts.id LIMIT 1) as url FROM adverts WHERE id_user = ?");
	...
	public function addAdvert($title, $category, $value, $description, $status) {
		...
		$sql = $this->db->prepare("INSERT INTO adverts (title, id_category, id_user, description, value, status) VALUES (?, ?, ?, ?, ?, ?)");
	...
	public function deleteAdvert($id) {
		...
		$sql = $this->db->prepare("DELETE FROM adverts_images WHERE id_advert = ?");
		$sql->execute(array($id));

		$sql = $this->db->prepare("DELETE FROM adverts WHERE id = ?");
		$sql->execute(array($id));
	}
	...
	public function getAdvert($id) {
		...
		*,
		...
		if($sql->rowCount() > 0) {
			...
			$sql = $this->db->prepare("SELECT id,url FROM adverts_images WHERE id_advert = ?");
			$sql->execute(array($id));
	...
	public function editAdvert($title, $category, $value, $description, $status, $photos, $id) {
		...
		$sql = $this->db->prepare("UPDATE adverts SET title = ?, id_category = ?, id_user = ?, description = ?, value = ?, status = ? WHERE id = ?");
		...
		if(count($photos) > 0) {
			for($i=0;$i<count($photos['tmp_name']);$i++) {
				...
				if(in_array($type, array('image/jpeg', 'image/png'))) {
					...
					$sql = $this->db->prepare("INSERT INTO adverts_images (id_advert, url) VALUES (?, ?)");
	...
	public function deletePhoto($id) {
		...
		$sql = $this->db->prepare("SELECT id_advert FROM adverts_images WHERE id = ?");
		$sql->execute(array($id));
		...
		$sql = $this->db->prepare("DELETE FROM adverts_images WHERE id = ?");
		$sql->execute(array($id));
